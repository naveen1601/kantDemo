...
//     const sortedLeaderboardBySequence = res.leaderboard_data.initialLeaderboard.sort((a, b) => parseFloat(a.sequence_value) - parseFloat(b.sequence_value));
...
export function getFinalleaderBoardMatrix(res, userId) {
  const sortedLeaderboardBySequence = res?.sort((a, b) => parseFloat(a.sequence_value) - parseFloat(b.sequence_value));
  let userOponentId = '';
  const studentArray = [];
  const pairingMatrix = [];
  ...
  for (let i = 0; i < sortedLeaderboardBySequence.length; i++) {
    ...
    if (studentArray.indexOf(sortedLeaderboardBySequence[i].student) < 0) {
      ...
      if (sortedLeaderboardBySequence[i].counterPlayer) {
        ...
        if (counterstudent.student == userId) {
          userOponentId = counterstudent.counterPlayer;
        }
